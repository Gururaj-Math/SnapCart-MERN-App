...
const EditProfile: React.FC = () => {
    ...
    const onFinish = async (values: any) => {
        try {
            ...
            const response = await axios.put(`${API_BASE_URL}users/update/${userId}`, updatedValues);
            const newUserData = {
                ...currentUser,
                avatar: form.getFieldValue('avatar'),
                coverImage: form.getFieldValue('coverImage'),
                bio: form.getFieldValue('bio'),
                links: [form.getFieldValue('link1'), form.getFieldValue('link2'), form.getFieldValue('link3')],
